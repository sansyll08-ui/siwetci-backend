...
const port = 3001;
const app = express();
const allowedOrigins = ['https://siwetci.web.app', 'http://localhost:3000'];

// Configuración de CORS
app.use(cors({
    origin: function (origin, callback) {
    // Permite peticiones sin origen (como herramientas de postman) o las que estén en la lista
...
// Aquí abajo debe seguir tu código actual (los app.post, app.get, etc.)

// ====================================================================
// 0. CONFIGURACIÓN DE BASE DE DATOS
// ====================================================================
const pool = new Pool({
    user: 'postgres',
    host: 'localhost',
...
// ====================================================================
// FUNCIÓN MAESTRA: REINICIAR CONTADOR SERIAL (OPTIMIZACIÓN PGADMIN)
// ====================================================================
...
/**
 * NUEVA LÓGICA: Carga la configuración de catálogos desde la tabla maestra 'ccatcatal'
 */
const inicializarCatalogos = async () => {
    try {
        // Seleccionamos con alias para asegurar que el objeto tenga las llaves correctas
        const query = `
            SELECT 
        ...
        console.log(`✅ Catálogos cargados: ${Object.keys(CATALOG_MAP).length}`);
    } catch (error) {
...
pool.connect().then(() => {
    console.log('✅ Servidor conectado a PostgreSQL');
    inicializarCatalogos();
});

/**
 * Lógica de apoyo: Busca una descripción o la inserta si no existe (usada en formularios rápidos)
 */
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        // 1. Buscamos al usuario incluyendo el Tipo de Persona en lugar del Rol
        const query = `
            SELECT u.*, 
        ...
        // --- REGLA 1: Fecha inferior a FecIni y Estado False ---
        if (!cuentaActiva && hoy < fIni) {
            return res.status(403).json({ 
                success: false, 
                message: "Cuenta desactivada, póngase en contacto con el administrador" 
            });
        }

        // --- REGLA 2: Fecha superior a FecFin (DESHABILITAR CUENTA) ---
        if (hoy > fFin) {
            await pool.query('UPDATE mUsuarios SET estatus = $1 WHERE id_usuario = $2', 
                [user.estatus === 'true' ? 'false' : 'inactivo', user.id_usuario]);
            
            if (!cuentaActiva) {
                return res.status(403).json({ success: false, message: "Cuenta Caducada, póngase en contacto con el administrador" });
            }
        ...
        // --- REGLA 3: Validar rango y estado True ---
        if (cuentaActiva && (hoy >= fIni && hoy <= fFin)) {
            // ACCESO CONCEDIDO
            return res.json({ 
                success: true, 
                username: user.username, 
                // Ahora roleName envía el Tipo de Persona para tu Menu.js
            ...
            return res.status(403).json({ 
                success: false, 
                message: "Acceso denegado: Su cuenta no está activa para este periodo." 
            });
        }
...
app.get('/api/catalogos', async (req, res) => {
    try {
        ...
        for (const [key, config] of Object.entries(CATALOG_MAP)) {
            try {
                // El error suele ocurrir aquí si config.desCol o config.table están mal
                const result = await pool.query(
                    `SELECT ${config.idCol} AS id, ${config.desCol} AS descripcion 
                        FROM ${config.table} 
                        ORDER BY ${config.desCol} ASC`
                );
                catalogos[key] = result.rows;
                listaParaSelector.push({ id: key, nombre: config.nombreVisual });
            } catch (innerError) {
                // ESTO TE DIRÁ EN LA TERMINAL QUÉ TABLA ESTÁ MAL
                console.error(`❌ Error en catálogo [${key}] (Tabla: ${config.table}):`, innerError.message);
                // Si un catálogo falla, continuamos con los demás para que no se muera todo el API
                continue; 
            }
        }
...
// ====================================================================
// 2. API: CRUD DINÁMICO DE CATÁLOGOS (CON REINICIO DE SERIAL)
// ====================================================================
...
app.get('/api/catalogos/verificar-uso/:tipo/:id', async (req, res) => {
    ...
    // Diccionario de integridad referencial
    const dependencias = {
    ...
    try {
        ...
        // Manejo especial para apellidos (paterno o materno)
...
app.post('/api/personas', async (req, res) => {
    ...
    try {
        ...
        const d = req.body;
        // Registro dinámico de sub-catálogos
...
// GUARDAR / MODIFICAR (Nuevo o Modif)
...
// BORRAR CON VALIDACIÓN (Regla del PDF)
app.delete('/api/aplicaciones/:id', async (req, res) => {
    const { id } = req.params;
    try {
        // 1. Verificar si está asignada a algún rol
        const check = await pool.query('SELECT * FROM mAcceRol WHERE id_app = $1', [id]);
        if (check.rows.length > 0) {
            return res.status(400).json({ 
                error: "La aplicación o módulo actual no puede ser eliminada, por estar asignada a una cuenta" 
            });
        }
        // 2. Si no está asignada, proceder a borrar
...
// --- GUARDAR ASIGNACIÓN DE ACCESOS (TRANSACCIÓN) ---
app.post('/api/asignar-privilegios', async (req, res) => {
    const { usuarios, aplicaciones } = req.body; // Arreglos de IDs
    const client = await pool.connect();

    try {
        await client.query('BEGIN');
        for (const userId of usuarios) {
            // 1. Limpiamos accesos previos para evitar duplicados si así lo requiere tu lógica
            await client.query('DELETE FROM mAccesos WHERE CvUser = $1', [userId]);
            
            // 2. Insertamos los nuevos accesos
            for (const appId of aplicaciones) {
                await client.query(
                    'INSERT INTO mAccesos (CvUser, CvAplicacion) VALUES ($1, $2)',
                    [userId, appId]
                );
            }
        }
...
app.get('/api/usuarios-disponibles', async (req, res) => {
    try {
        const result = await pool.query('SELECT id_usuario as id, username as name FROM mUsuarios WHERE estatus = $1', ['activo']);
        res.json(result.rows); // Esto envía el JSON que React espera
    } catch (err) {
        console.error(err);
...
app.get('/api/aplicaciones', async (req, res) => {
    try {
        const result = await pool.query('SELECT id_app as clave, nombre_app as descripcion FROM mAplicaciones');
        res.json(result.rows);
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: "Error en el servidor" });
...
app.put('/api/usuarios/cambiar-password', async (req, res) => {
    ...
    try {
        // 1. Validar que el password anterior sea el correcto del usuario actual
        const checkUser = await pool.query(
            'SELECT password FROM mUsuarios WHERE username = $1',
            [username]
        );
        ...
        // 2. Validar que el nuevo password NO sea usado por NADIE más en la tabla
        const checkDuplicate = await pool.query(
            'SELECT username FROM mUsuarios WHERE password = $1',
            [newPass] // El nuevo password que el usuario quiere poner
        );

        if (checkDuplicate.rows.length > 0) {
            // Si la consulta trae algo, significa que alguien ya lo tiene
            return res.status(400).json({ 
                success: false, 
                message: "Cambio de Password Inconcluso, póngase en contacto con el administrador o reintentelo." 
            });
        }

        // 3. Si todo es correcto, actualizar
        await pool.query(
            'UPDATE mUsuarios SET password = $1 WHERE username = $2',
            [newPass, username]
        );
...
// ====================================================================
// 5. API: MANTENIMIENTO DE USUARIOS (NUEVO)
// ====================================================================
// --- OBTENER USUARIOS PARA LA REJILLA (Decodificados) ---
// 1. Obtener listado de usuarios para la tabla principal
...
// 2. Obtener personas para el ComboBox (Solo personas sin usuario asignado)
...
app.post('/api/usuarios/gestion', async (req, res) => {
    ...
    try {
        ...
        if (modo === 'NUEVO') {
            const query = `
                INSERT INTO mUsuarios (id_persona, username, password, fecha_inicio, fecha_fin, estatus)
                VALUES ($1, $2, $3, $4, $5, $6)`;
            await pool.query(query, [id_persona, login, password, fecini, fecfin, edocta]);
        } else {
            const query = `
                UPDATE mUsuarios 
                SET id_persona = $1, username = $2, password = $3, fecha_inicio = $4, fecha_fin = $5, estatus = $6
                WHERE id_usuario = $7`;
        ...
        console.error("❌ Error en Gestion Usuarios:", error.message);
        res.status(500).json({ success: false, message: "Error interno del servidor." });
    }
});

// 4. Verificar integridad en mAccesos antes de eliminar
...
app.get('/api/usuarios/:id/validar-borrado', async (req, res) => {
    try {
        ...
        const tieneAccesos = parseInt(result.rows[0].count) > 0;
        
        res.json({ tieneAccesos: tieneAccesos });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
app.post('/api/usuarios/cambiar-password', async (req, res) => {
    ...
    try {
        // Validar que el password no esté en uso por otro usuario (Regla de seguridad)
...
// Obtener el resumen de caja del día actual (Sincronizado con esquema real)
app.get('/api/corte/resumen', async (req, res) => {
    try {
        // Sumamos ventas en EFECTIVO (id_estatus = 1)
        const efectivoRes = await pool.query(`
            SELECT COALESCE(SUM(total_venta), 0) as total 
            FROM mventas 
            WHERE DATE(fec_venta) = CURRENT_DATE AND id_estatus = 1
        `);

        // Sumamos ventas en TARJETA (id_estatus = 2)
        const tarjetaRes = await pool.query(`
            SELECT COALESCE(SUM(total_venta), 0) as total 
            FROM mventas 
            WHERE DATE(fec_venta) = CURRENT_DATE AND id_estatus = 2
        `);

        // Sumamos ventas en TRANSFERENCIA (id_estatus = 3)
        const transfRes = await pool.query(`
            SELECT COALESCE(SUM(total_venta), 0) as total 
            FROM mventas 
            WHERE DATE(fec_venta) = CURRENT_DATE AND id_estatus = 3
        `);
...
// Guardar auditoría del cierre de caja
app.post('/api/corte/finalizar', async (req, res) => {
    ...
    try {
        ...
        await pool.query(query, [
            id_usuario, fondoCaja, ventasEfectivo, 
            totalEsperado, efectivoReal, diferencia, observaciones
        ]);
...
app.listen(port, () => console.log(`🚀 Servidor Montero's Profesional activo en http://localhost:${port}`));